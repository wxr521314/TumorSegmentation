...
class DenseUNet(BasicModule):
    def __init__(self):
        super(DenseUNet, self).__init__()
        self.model_name = 'dense-unet'
        ...
        self.stage1_conv2 = nn.Sequential(
            nn.Conv3d(in_channels=1 + 16, out_channels=16, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=16)
        )
        ...
        self.stage2_conv2 = nn.Sequential(
            nn.Conv3d(in_channels=32 + 32, out_channels=32, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=32)
        )
        self.stage2_conv3 = nn.Sequential(
            nn.Conv3d(in_channels=32 + 32 + 32, out_channels=32, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=32)
        )
        ...
        self.stage3_conv2 = nn.Sequential(
            nn.Conv3d(in_channels=64 + 64, out_channels=64, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=64)
        )
        self.stage3_conv3 = nn.Sequential(
            nn.Conv3d(in_channels=64 + 64 + 64, out_channels=64, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=64)
        )
        ...
        self.stage4_conv2 = nn.Sequential(
            nn.Conv3d(in_channels=128 + 128, out_channels=128, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=128)
        )
        self.stage4_conv3 = nn.Sequential(
            nn.Conv3d(in_channels=128 + 128 + 128, out_channels=128, kernel_size=3, stride=1, padding=1),
            nn.PReLU(num_parameters=128)
        )
